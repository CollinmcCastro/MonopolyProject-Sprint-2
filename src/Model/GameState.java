...
public class GameState {
    private final List<Model.Space> spaces; // Board spaces
    private final Stack<Model.ChanceCard> chanceDeck; // Chance card deck
    private final Stack<Model.CommunityChestCard> communityDeck; // Community Chest card deck

    public GameState(List<Model.Space> spaces, Stack<Model.ChanceCard> chanceDeck, Stack<Model.CommunityChestCard> communityDeck) {
        this.spaces = spaces;
    ...
    void initializeChanceCards() {
        ...
        chanceDeck.add(new ChanceCard("Advance to the nearest Railroad. If unowned, you may buy it from the Bank. If owned, pay owner twice the rental to which they are otherwise entitled.", player -> ChanceCard.moveToNearestRailroad()));
        chanceDeck.add(new ChanceCard("Advance to the nearest Railroad. If unowned, you may buy it from the Bank. If owned, pay owner twice the rental to which they are otherwise entitled.", player -> ChanceCard.moveToNearestRailroad()));
        chanceDeck.add(new ChanceCard("Advance token to nearest Utility. If unowned, you may buy it from the Bank. If owned, throw dice and pay owner a total ten times amount thrown.", player -> ChanceCard.moveToNearestUtility()));
        ...
        chanceDeck.add(new ChanceCard("Make general repairs on all your property. For each house pay $25. For each hotel pay $100.", player -> ChanceCard.makeGeneralRepairs()));
        ...
        chanceDeck.add(new ChanceCard("You have been elected Chairman of the Board. Pay each player $50.", player -> ChanceCard.payEachPlayer()));
    ...
    void initializeCommunityChestCards() {
        ...
        communityDeck.add(new CommunityChestCard("It is your birthday. Collect $10 from every player.", ignored -> CommunityChestCard.collectFromEachPlayer()));
